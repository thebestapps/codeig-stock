...
require_once(dirname(__FILE__) . "/src/CapabilityProfile.php");
require_once(dirname(__FILE__) . "/src/EscposImage.php");
require_once(dirname(__FILE__) . "/src/GdEscposImage.php");
...
class Escpos
{
     ...
     */
    public function pdf417Code($content, $width = 3, $heightMultiplier = 3, $dataColumnCount = 0, $ec = 0.10, $options = self::PDF417_STANDARD)
    {
    ...
    public function selectPrintMode($mode = self::MODE_FONT_A)
    {
        ...
        $this -> connector -> write(self::ESC . "!" . chr($mode)); //sets print mode for normal text, like dbl height, dbl width, etc
    }
    
	public function selectChinesePrintMode($mode = self::MODE_FONT_A)
    {
        $allModes = self::MODE_FONT_B | self::MODE_EMPHASIZED | self::MODE_DOUBLE_HEIGHT | self::MODE_DOUBLE_WIDTH | self::MODE_UNDERLINE;
        if (!is_integer($mode) || $mode < 0 || ($mode & $allModes) != $mode) {
            throw new InvalidArgumentException("Invalid mode");
        ...
        $this -> connector -> write(self::ESC . "!" . chr($mode)); //sets print mode for normal text, like dbl height, dbl width, etc
		$this -> connector -> write(self::FS . "!" . chr(12)); //sets print mode for Chinese text, like dbl height, dbl width, etc
    }
    ...
    public function setPrintWidth($width = 512)
    {
        self::validateInteger($width, 1, 65535, __FUNCTION__);
         $this -> connector -> write(self::GS . 'W' . self::intLowHigh($width, 2));
    }
