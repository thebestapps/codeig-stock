...
;(function ( $, window, document, undefined ) {
		...
		var layouts = {
			selectable: ['english', 'email', 'virtual_table'],
			/*
			azeri: [
			...
			*/
			english: [
				['q','w','e','r','t','y','u','i','o','p', ],
				['a','s','d','f','g','h','j','k','l', ],
				['shift','z','x','c','v','b','n','m', 'backspace', 'clear'],
				['numeric_switch','layout_switch', 'space','return']
			],
			email:[
				['q','w','e','r','t','y','u','i','o','p', ],
				['a','s','d','f','g','h','j','k','l', ],
				['shift','z','x','c','v','b','n','m', 'backspace', 'clear'],
				['@','btngmail','btnyahoo','btncom', 'backspace'],
				['numeric_switch','layout_switch', 'space','return']
			],
			/*
			russian: [
		...
		Plugin.prototype = {
				...
				events: function(){
					var letters = $(this.element).find('.letter'),
						...
						backspace2_key = $(this.element).find('.backspace2'),
						return_key = $(this.element).find('.return'),
						btngmail_key = $(this.element).find('.btngmail'),
						btnyahoo_key = $(this.element).find('.btnyahoo'),
						btncom_key = $(this.element).find('.btncom'),
					...
					btngmail_key.on('click', function(){
						me.type((shift || capslock) ? $(this).text().toUpperCase() : $(this).text());
					})

					btnyahoo_key.on('click', function(){
						me.type((shift || capslock) ? $(this).text().toUpperCase() : $(this).text());
					})

					btncom_key.on('click', function(){
						me.type((shift || capslock) ? $(this).text().toUpperCase() : $(this).text());
					})
					...
					backspace2_key.on('click', function(){
						me.backspace();
					})
				...
				type: function(key){
					...
					var KeyboardInputLimit = document.getElementById('search_field').getAttribute("maxlength");
					if(textLength != KeyboardInputLimit){
