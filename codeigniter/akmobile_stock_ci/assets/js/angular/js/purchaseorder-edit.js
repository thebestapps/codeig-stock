(function() {
    ...
    .controller("akamaiposController", ['$scope', '$compile', '$http', 'svc', '$routeParams', '$q', 'posData', '$window', '$filter', function ($scope, $compile, $http, svc, $routeParams, $q, posData, $window, $filter) {
		...
		$("#SelectSupplier").jqxComboBox({placeHolder: "Select Supplier" });
		setTimeout(function(){
		...
		var POInfo = function(){
			$http({
				method: 'get',
				url: api_url+'purchaseheader-information/'+$scope.purchaseheader
			}).success(function(data){
				var POUnique = data[0].unique;
				if(data[0].supplier_unique){
					POSupplier = data[0]['supplier_unique'].Company;
				}else{
					POSupplier = 'No Supplier';
				}
				var PORef = data[0].po;
				
				$scope.itemdesc = {
					info: "ID: "+POUnique+" | "+POSupplier + " | "+PORef
				}
        ...
        $scope.CreateNew = function(){
            ...
            .success(function(data){
                $window.location.href = base_url+"backoffice/purchase-order/add/"+data.unique;
            });
        };
        ...
        $scope.DeletePO = function(){
            ...
            $scope.delete = {
                message: "Are you sure you want to delete Purchase Order "+$scope.purchaseheader+"?"
            }
        };


        $scope.DeleteYes = function(){
            var POUnique = $scope.purchaseheader;
            $http({
                method: 'put',
